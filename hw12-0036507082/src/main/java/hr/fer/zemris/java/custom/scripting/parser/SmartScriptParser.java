...
/**
 * Class SmartScriptParser is parser class
 * for parsing given text with lexer help.
 * 
 * @author Filip
 */
public class SmartScriptParser {
	...
	/**
	 * This method is used for parsing text
	 * with lexer help.
	 * 
	 */
	private void parse() {
		while (true) {
			...
			if (lexer.getState() == SmartScriptStates.TEXT) {
				...
				if (token.getType() == SmartScriptTokenType.EOF) {
					if (stack.size() != 1) throw new SmartScriptParserException("Invalid text.");
				...
				else throw new SmartScriptParserException("Invalid SmartScriptToken.");
			}
			else if (lexer.getState() == SmartScriptStates.TAG) {
				
				if (token.getType() == SmartScriptTokenType.EOF) throw new SmartScriptParserException("Tag is not closed.");
				else if (token.getType() == SmartScriptTokenType.TEXT) throw new SmartScriptParserException("Invalid SmartScriptToken.");
				else if (token.getType() == SmartScriptTokenType.TAG) {
					...
					else throw new SmartScriptParserException("Invalid TAGNAME.");
				}
				else throw new SmartScriptParserException("Invalid TAGNAME.");
			}
			else throw new SmartScriptParserException("Invalid Parser state.");
		} 
	}
	...
	private void parseFor() {
		int i = 0;
		token = lexer.nextToken();
		if (token.getType() != SmartScriptTokenType.VARIABLE) throw new SmartScriptParserException("Error in for.");
		...
		while (true) {
			...
			else throw new SmartScriptParserException("Error in for."); 
		}if (elements.length < 2) throw new SmartScriptParserException("Error in for."); 
